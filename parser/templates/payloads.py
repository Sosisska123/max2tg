...
def get_token_json(token: str) -> str:
    ...
    Args:
        token (str): You can get the token at the end of authentication (OPCODE 18). You can get it after sending your phone number (OPCODE 17) and sending a received mini-token again to the server (OPCODE 18)
...
def get_subscribe_json(state: bool, chat_id: str, seq: int) -> str:
    """OPCODE 75
    Somewhy when a user changes the chat this package is sent. honestly idk why and what does it affect
...
def get_checkcode_json(token: str, code: str, seq: int) -> str:
    """OPCODE 18
    Used for check code
